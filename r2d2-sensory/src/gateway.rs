...
impl SensoryGateway {
    ...
    pub async fn ingest(&self, stimulus: Stimulus) -> Result<Fragment<Signal>> {
        info!("SENSORY GATEWAY : Réception d'un stimulus de type {:?}", stimulus.stimulus_type);
        ...
        info!("-> Délégation à l'agent Cortical (Whisper/LLaVA en attente)... ({})", system_prompt);
        
        // Simulation d'un retour JSONAI brut non validé (Pour la phase de test)
        let synthetic_jsonai = format!(r#"{{
            "id": "stimulus-{}",
