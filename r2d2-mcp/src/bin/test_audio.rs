...
async fn main() {
    ...
    println!("📀 [PRE-PROCESSING] Fichier cible (Haute Densité) : {}", target_file);
    ...
    let fragments = chunker.chunk(&parent_stimulus).expect("Échec du hachage audio !");
    
    println!("📦 [PRE-PROCESSING] {} fragments de 30s purs ont été extraits en f32 binaire.", fragments.len());

    let mut agent = AudioAgent::new();
    println!("🔌 [CORTEX] Chargement de l'agent (Whisper) en VRAM...");
    agent.load().await.expect("Le chargement des tenseurs Whisper a échoué.");

    for (i, frag) in fragments.iter().enumerate() {
        println!("\n🎙️ [INFERENCE] Ingestion du fragment {}/{} ({})", i+1, fragments.len(), frag.payload_path.display());
        let result = agent.generate_thought(&frag.payload_path.to_string_lossy()).await;
