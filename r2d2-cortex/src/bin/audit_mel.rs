...
fn main() {
    ...
    let repo = api.repo(Repo::with_revision("openai/whisper-tiny".to_string(), RepoType::Model, "main".to_string()));
    ...
    println!("Énergie absolue (Somme abs) dans [0..80] : {:.4}", energy_in_first_80);
    println!("Énergie absolue (Somme abs) dans indices [0, frames, 2*frames... 79*frames] : {:.4}", energy_in_first_frames);
