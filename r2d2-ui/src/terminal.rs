...
pub fn spawn_terminal(
    ...
    if let Some(id) = workspace_id {
        ...
        let mut container_name = format!("r2d2-workspace-{}", id); // fallback
        ...
        if let Some(s) = &session_opt {
            if let Some(repo_name) = s.github_sources.first() {
                ...
                workdir = format!("/home/{}", folder);
                container_name = format!("r2d2-workspace-{}", repo_name.replace("/", "_"));
            } else {
                container_name = "r2d2-workspace-generic".to_string();
            }
        }
        ...
        let bash_entrypoint = format!("cd {} 2>/dev/null || cd /home; exec /bin/bash", workdir);
        ...
        return Err(anyhow::anyhow!(
            "Sécurité R2D2: Accès au shell local interdict. Une session Workspace est obligatoire."
        ));
    }
...
pub async fn handle_terminal_socket(socket: WebSocket, session_id: String) {
    println!(
        ">>> HANDLE TERMINAL SOCKET INVOCATION for session_id: {}",
        session_id
    );
    let workspace_id = if session_id == "local" || session_id.is_empty() {
        None
    } else {
...
pub async fn handle_ai_terminal_socket(
    socket: WebSocket,
    session_id: String,
    mut terminal_rx: tokio::sync::broadcast::Receiver<(String, String)>,
) {
    let (mut ws_sender, mut ws_receiver) = socket.split();
    ...
    loop {
        tokio::select! {
            Ok((target_session, msg)) = terminal_rx.recv() => {
                if target_session == session_id && ws_sender.send(Message::Binary(msg.into_bytes())).await.is_err() {
                    break;
                }
            }
            msg = ws_receiver.next() => {
...
mod tests {
    ...
    #[test]
    fn test_spawn_terminal_local_not_allowed() {
        let result = spawn_terminal(None);
        match result {
            Err(e) => assert!(e
                .to_string()
                .contains("Sécurité R2D2: Accès au shell local interdict")),
            Ok(_) => panic!("Dovrait retourner une erreur de sécurité"),
        }
    }
}
